...
existing_research_body=""
if [ -f "$resources_file" ]; then
  existing_research_body="$(
    awk '
      seen { print }
      /^Arcaneum status during init:/ { seen = 1; next }
    ' "$resources_file" | sed '/./,$!d'
  )"
fi
...
- `KATA_FLIGHT_HOME`: Kata Flight skill/tooling checkout.
- `KATA_FLIGHT_CONSUMER_ROOT`: repo whose code and kata tracker are changed.
- `KATA_FLIGHT_CONTEXT_ROOT`: repo or sibling checkout that owns project context
  (`context/`) and RDR evidence (`rdr/evidence/`).

Kata Flight itself needs no evidence directory.
EOF
...
if [ -n "$existing_research_body" ]; then
  printf '\n%s\n' "$existing_research_body" >> "$resources_file"
else
  cat >> "$resources_file" <<'EOF'
